...
async function proxyRawFile(path) {
  ...
  return new Response(await resp.text(), {
    status: 200,
    headers: { "Content-Type": guessContentType(path), "Cache-Control": "max-age=300" },
  });
}
...
async function handleSignup(request, env, url) {
  ...
  const emailRecord = { email, feeds, token, created: now, source: "subscribe_page", syncCount: 0, lastSync: null, status: "active" };
  const subscriberRecord = { token, email, note: "Self-signup", feeds, created: now, status: "active", lastSync: null, syncCount: 0 };
...
async function handleTokenFeed(request, env, url) {
  if (!env.ANIME_CAL) return new Response("KV binding ANIME_CAL is missing.", { status: 503 });
  ...
  const validFeeds = new Set(["master","crunchyroll","netflix","hidive","manga","manhwa","manhua"]);
  const feedName = validFeeds.has(requestedFeed) ? requestedFeed : "master";
  const mappedEmail = await env.ANIME_CAL.get(tokenKey(token));
  if (!mappedEmail) return new Response("Invalid or expired subscription link.", { status: 403 });
  const emailRecordRaw = await env.ANIME_CAL.get(emailKey(mappedEmail));
  if (!emailRecordRaw) return new Response("Subscription record not found.", { status: 403 });
  const emailRecord = JSON.parse(emailRecordRaw);
  if (emailRecord.status && emailRecord.status !== "active") return new Response("This subscription has been deactivated.", { status: 403 });
  const feedResp = await fetchRaw("/feeds/" + feedName + ".ics");
  if (!feedResp.ok) return new Response("Calendar temporarily unavailable.", { status: 503 });
  const now = new Date().toISOString();
...
async function handleAdminAPI(request, env, path, url) {
  if (!env.ANIME_CAL) return json({ error: "KV binding ANIME_CAL is missing." }, 503);
  if (!env.ADMIN_PASSWORD) return json({ error: "ADMIN_PASSWORD secret is missing." }, 503);
  ...
  if (path === "/admin/api/stats" && request.method === "GET") {
    ...
    const subs = await Promise.all(subsList.keys.map(async k => { const v = await env.ANIME_CAL.get(k.name); return v ? JSON.parse(v) : null; }));
    const f = subs.filter(Boolean);
    return json({ totalSubscribers: f.length, activeSubscribers: f.filter(s=>s.status==="active").length, inactiveSubscribers: f.filter(s=>s.status!=="active").length, totalSyncs: f.reduce((sum,s)=>sum+(s.syncCount||0),0), totalEmails: emailsList.keys.length });
  }

  if (path === "/admin/api/subscribers" && request.method === "GET") {
    const list = await env.ANIME_CAL.list({ prefix: "sub:" });
    const subs = await Promise.all(list.keys.map(async k => { const v = await env.ANIME_CAL.get(k.name); return v ? JSON.parse(v) : null; }));
    return json({ subscribers: subs.filter(Boolean).sort((a,b)=>String(b.created||"").localeCompare(String(a.created||""))) });
  }
  ...
  if (path === "/admin/api/subscribers" && request.method === "POST") {
    ...
    await env.ANIME_CAL.put(subKey(token), JSON.stringify(sub));
    if (email) {
      await env.ANIME_CAL.put(emailKey(email), JSON.stringify({ email, feeds, token, created: now, source: "admin_create", syncCount: 0, lastSync: null, status: "active" }));
      await env.ANIME_CAL.put(tokenKey(token), email);
    }
  ...
  if (subMatch && request.method === "DELETE") {
    ...
    if (existing) {
      ...
      if (sub.email) await env.ANIME_CAL.delete(emailKey(sub.email));
  ...
  if (path === "/admin/api/emails" && request.method === "GET") {
    ...
    const emails = await Promise.all(list.keys.map(async k => { const v = await env.ANIME_CAL.get(k.name); return v ? JSON.parse(v) : null; }));
    const filtered = emails.filter(Boolean).sort((a,b)=>String(b.created||"").localeCompare(String(a.created||"")));
  ...
  if (path === "/admin/api/emails/export" && request.method === "GET") {
    ...
    const rows = [["Email","Feeds","Created","Last Sync","Sync Count","Status","Token"],...filtered.map(e=>[e.email||"",(e.feeds||[]).join("|"),e.created||"",e.lastSync||"",e.syncCount||0,e.status||"",e.token||""])];
